...
local function get_enum_const_go(bufnr, const_node)
  ...
  if const_node:named_child_count() < 2 then
    vim.g.exemplum.logger:error(
      "The const assignment in the current cursor position is not an enum assignment"
    )
    return ""
  end
...
local function get_enum_chunk(bufnr, filetype)
  ...
  if current_node:type() == enum_node_name then
    ...
    if not current_node then
      vim.g.exemplum.logger:error(
        "Could not find an enum in the current scope: probably your cursor is placed in the wrong scope?"
      )
      return {}
    end
...
---Refactor an enum at the cursor position
---@param bang boolean Whether the `:Exemplum` command ran with bang (`!`)
local function refactor_enum(bang)
  ...
  if #enum_range == 0 then return {} end
